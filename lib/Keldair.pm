...
use constant {
    VERSIONSTRING => '3.0.0-alpha1',
    VERSION       => 3,
    SUBVERSION    => 0,
    REVISION      => 0,
    RELEASESTAGE  => q{alpha},
    RELEASE       => 1,
};

our $VERSION = '3.0.0-alpha1';
...
sub new {
    ...
    _connect( config('server/host'), config('server/port') )
      or croak("Error connecting to server: $!\n");
    return 1;
}
...
sub _connect {
    ...
    Keldair::connect( config('keldair/user'), config('keldair/real'),
        config('keldair/nick') );
    _loop
      or croak(
